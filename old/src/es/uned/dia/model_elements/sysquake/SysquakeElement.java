...
public class SysquakeElement implements ModelElement {
	...
	static ImageIcon ELEMENT_ICON = ResourceLoader.getIcon("es/uned/dia/model_elements/sysquake/sysquake.png"); // This icon is included in this jar
	...
	private DefaultTableModel sysquakeTableModel;
	...
	//Default options of Sysquake Element
	...
	public String getGenericName() { return "sysquake"; }

	public ImageIcon getImageIcon() { return ELEMENT_ICON; }

	public String getConstructorName() {						
		 return "es.uned.dia.model_elements.sysquake.SysquakeWrapper";             
	}

	public String getInitializationCode(String _name) {	
		String _code;
		//Create Sysquake constructor only once						
	    //_code= "if ("+_name+"==null) "+_name + " = new " + getConstructorName() + "(\""+sqlinkFile+"\");\n";
		_code= "if ("+_name+"==null) "+_name + " = new " + getConstructorName() + "();\n";

		//Check Linked Variables
		  String[] variables;
		  String sysquakeVariable="";
		  String ejsVariable="";
		  boolean firstLink=true;	
		  for (Iterator<String> e = variablesTable.listIterator(); e.hasNext(); ) {
			  variables=e.next().split(",");
			  if (variables.length<2) continue;
			  sysquakeVariable=variables[0];
			  ejsVariable=variables[1];
			  if ((sysquakeVariable.trim()!="" || sysquakeVariable.trim()!=null) && (ejsVariable.trim()!="" || ejsVariable.trim()!=null)) {
					if (firstLink) {
						_code=_code+_name+".setClient(_model);\n";
						firstLink=false;
					}
					_code=_code+_name+".linkVariables(\""+ejsVariable.trim()+"\",\""+sysquakeVariable.trim()+"\");\n";						 
			  }		
          }	
	...
	public String getResourcesRequired() {
		//Get resource and copy it into /extensions
		if (!extractResourcesFromJar("com/calerga/sysquake","SysquakeLink.dll")) return null;
		
		//Copy resource from /extensions to /workspace/source		
		org.colos.ejs.library.utils.TemporaryFilesManager.extractToDirectory("SysquakeLink.dll", new File(getWorkSpacePath()+"source/"), true);		   
		return "SysquakeLink.dll";		   		  		 
	}
	...
	public String getTooltip() {
		return "encapsulates an object that connects EJS to Sysquake";
	}
	...
	public void showEditor(String _name, Component _parentComponent, ModelElementsCollection _collection) {
		...
		if (editorDialog==null) { // create the dialog
			...
			field.getDocument().addDocumentListener (new DocumentListener() {
				public void changedUpdate(DocumentEvent e) { elementsCollection.reportChange(SysquakeElement.this); }
				public void insertUpdate(DocumentEvent e)  { elementsCollection.reportChange(SysquakeElement.this); }
				public void removeUpdate(DocumentEvent e)  { elementsCollection.reportChange(SysquakeElement.this); }
			});
			...
			sysquakeTableModel = new DefaultTableModel();
			sysquakeTableModel.addColumn("Sysquake Variable");
			sysquakeTableModel.addColumn("EJS Variable");
			table=new JTable(sysquakeTableModel);  			
			TableModelListener ta=new TableModelListener (){ //Add a row to the table when last row is edited
				public void tableChanged(TableModelEvent e){
					if (TableModelEvent.UPDATE==e.getType() && table.getRowCount()==e.getLastRow()+1){
						sysquakeTableModel.addRow(new String[]{"",""});
					}    		
				}    	  
			};
			...
			popupMenu.add(new AbstractAction("Insert"){
				...
				public void actionPerformed(ActionEvent e) {
					if (table.getSelectedRow()>=0 && table.getSelectedRow()<table.getRowCount())
						sysquakeTableModel.insertRow(table.getSelectedRow()+1,new String[]{"",""}); 
				}
			});
			popupMenu.add(new AbstractAction("Delete"){
				...
				public void actionPerformed(ActionEvent e) {
					if (table.getSelectedRow()>=0 && table.getSelectedRow()<table.getRowCount() && table.getRowCount()>1)
						sysquakeTableModel.removeRow(table.getSelectedRow()); 
				}
			});
			...
			editorDialog.addWindowListener(new WindowAdapter(){	        
	          public void windowClosing(WindowEvent we){	 
	        	...
	        	variablesTable.clear();
	        	String ejsVariable,externalVariable;
	  		    for (int i=0; i<table.getRowCount();i++){	  		    	
	  		    	ejsVariable="";
	  		    	externalVariable="";	  		    	
	  		    	if (((String)table.getValueAt(i, 0))!=null && ((String)table.getValueAt(i, 0))!="") externalVariable=((String)table.getValueAt(i, 0)); 
	  		   	    if (((String)table.getValueAt(i, 1))!=null && ((String)table.getValueAt(i, 1))!="") ejsVariable=((String)table.getValueAt(i, 1));	  		    		
	  		   	    variablesTable.add(externalVariable+","+ejsVariable);
	  			}	   	  		    	         
	          }
	        });
	...
	static private Component createHelpComponent() {
		...
		try { // read the help for this element
			java.net.URL htmlURL = ResourceLoader.getResource("es/uned/dia/model_elements/sysquake/Sysquake.html").getURL();
	...
	private void initiateDialogData(){		
        //Update Dialog
			...
			for (Iterator<String> e = variablesTable.listIterator(); e.hasNext(); ) {
				variables=e.next().split(",");
				sysquakeTableModel.addRow(variables);  
			} 				
