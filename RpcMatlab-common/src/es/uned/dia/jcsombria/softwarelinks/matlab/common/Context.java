...
public class Context {
	...
	protected java.util.Map<String, List> history = new java.util.HashMap<>();
	protected int MAX_BUFFER_SIZE = 5000;
	...
	private boolean addLink(Link link) {
		...
		history.put(variable, new ArrayList());
	...
	public void getValues() {
		for(String name: history.keySet()) {
			...
			if(links.containsKey(name)) {
				...
				String variable = link.getNameOfClientVariable();
				value = getServerValue(variable);
				setClientValue(variable, value);
			} else {
				value = ((double[])matlab.get(name))[0];
			}
			List var_history = history.get(name);
			if(var_history != null) {
				var_history.add(value);
				if(var_history.size() > MAX_BUFFER_SIZE) {
					var_history.remove(0);
				}
			}
		}
//		for(Link link: links.values()) {
//			String variable = link.getNameOfClientVariable();
//			Object value = getServerValue(variable);
//			setClientValue(variable, value);
//		}
	}
	...
	public List getHistory(String variable) {
